...
func (w *WebServer) registerRoutes(ctrl Controller) {
	w.router.GET(betByIdPath, ctrl.BetByIdHandler())
	w.router.GET(betsByUserPath, ctrl.BetsByUserHandler())
	w.router.GET(betsByStatusPath, ctrl.BetsByStatusHandler())
}

// Controller handles api calls
type Controller interface {
	BetByIdHandler() gin.HandlerFunc
	BetsByUserHandler() gin.HandlerFunc
	BetsByStatusHandler() gin.HandlerFunc
}
