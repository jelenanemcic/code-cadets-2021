...
import (
	...
	domainmodels "github.com/jelenanemcic/code-cadets-2021/homework_3/calculator/internal/domain/models"
	storagemodels "github.com/jelenanemcic/code-cadets-2021/homework_3/calculator/internal/infrastructure/sqlite/models"
...
func (r *BetRepository) InsertBet(ctx context.Context, bet domainmodels.Bet) error {
	...
	if err != nil {
		return errors.Wrap(err, "bets repository failed to insert a bet with id "+bet.Id)
	}
	return nil
}
...
// UpdateBet updates the provided calculated bet in the database. An error is returned if the operation
// has failed.
func (r *BetRepository) UpdateBet(ctx context.Context, bet domainmodels.Bet) error {
	storageBet := r.betMapper.MapDomainBetToStorageBet(bet)
	err := r.queryUpdateBet(ctx, storageBet)
	if err != nil {
		return errors.Wrap(err, "bets repository failed to update a bet with id "+bet.Id)
	}
	return nil
}
...
func (r *BetRepository) GetBetByID(ctx context.Context, id string) (domainmodels.Bet, bool, error) {
	...
	if err != nil {
		return domainmodels.Bet{}, false, errors.Wrap(err, "bets repository failed to get a bet with id "+id)
	}
