...
import (
	...
	"github.com/jelenanemcic/code-cadets-2021/homework_3/calculator/internal/infrastructure/rabbitmq/models"
...
func (p *BetPublisher) Publish(ctx context.Context, bets <-chan models.BetCalculated) {
	go func() {
		for bet := range bets {
			select {
			case <-ctx.Done():
			...
			default:
				...
				if err != nil {
					log.Println("Failed to marshal the following calculated bet:", bet)
					continue
				}
				...
				if err != nil {
					log.Println("Failed to publish the following calculated bet:", betJson)
				}
			}
		}
	}()
